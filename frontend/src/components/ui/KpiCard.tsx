...
export default function KpiCard({ label, value, sub, accent = 'amber' }: Props) {
  return (
    <div className="bg-bg-surface border border-border rounded-xl p-4 md:p-5 relative overflow-hidden hover:border-border-2 transition-colors">
      <div className={`absolute top-0 left-0 right-0 h-0.5 bg-gradient-to-r ${accents[accent]}`} />
      <div className="text-[10px] font-semibold uppercase tracking-widest text-text-3 mb-2 md:mb-3">{label}</div>
      <div className="font-mono text-2xl md:text-3xl font-medium text-text leading-none mb-1">{value}</div>
