  ...
  const columns = [
    {
      ...
      render: (p: Product) => (
        <div className="flex items-start gap-2">
          {isLow(p) && <span title="Stoc sub minim" className="mt-0.5 shrink-0 text-xs">{isCritical(p) ? '🔴' : '🟡'}</span>}
          <div>
    ...
    { key: 'unit', header: 'U.M.', render: (p: Product) => <span className="text-text-2">{unitLabel(p.unit)}</span> },
    {
      ...
      render: (p: Product) => {
        ...
        return (
          <span className={`font-mono text-base font-medium ${isCritical(p) ? 'text-danger' : isLow(p) ? 'text-accent' : 'text-success'}`}>
            {total}
          </span>
        )
      },
      ...
      render: (p: Product) => (
        <div className="flex items-center gap-1.5">
          ...
          {isLow(p) && p.minStock > 0 && (
            <span className="text-[10px] bg-danger/10 text-danger px-1.5 py-0.5 rounded-full font-medium">sub minim</span>
          )}
        </div>
      ),
    },
    ...
    },
    {
      ...
      render: (p: Product) => (
        <div className="flex gap-1.5">
          <Button size="sm" variant="ghost" onClick={() => navigate(`/products/${p.id}/history`)} title="Istoric mișcări">📋</Button>
          {canEdit && <>
            <Button size="sm" onClick={() => openEdit(p)}>✎</Button>
            <Button size="sm" variant="danger" onClick={() => { if (confirm(`Ștergi "${p.name}"?`)) deleteMutation.mutate(p.id) }}>✕</Button>
          </>}
        </div>
      ),
    },
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <Button variant="ghost" size="sm" onClick={() => exportProductsExcel(filtered)} disabled={filtered.length === 0}>⬇ Excel</Button>
          <Button variant="ghost" size="sm" onClick={() => exportProductsPdf(filtered)} disabled={filtered.length === 0}>⬇ PDF</Button>
      ...
      {lowCount > 0 && (
        <div className="flex items-center justify-between bg-danger/5 border border-danger/15 rounded-xl px-4 py-3 mb-4 gap-3">
          ...
          <button onClick={() => handleAlertOnly(!alertOnly)}
            className={`text-xs px-3 py-1.5 rounded-md transition-colors whitespace-nowrap ${alertOnly ? 'bg-danger/20 text-danger font-medium' : 'bg-danger/10 text-danger hover:bg-danger/20'}`}>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        ...
        <select value={typeFilter} onChange={e => handleTypeFilter(e.target.value)}
          ...
          <option value="MATERIE_PRIMA">{PRODUCT_TYPE_ICONS['MATERIE_PRIMA']} Materie Primă</option>
          <option value="GATA_ASAMBLARE">{PRODUCT_TYPE_ICONS['GATA_ASAMBLARE']} Kit Asamblare</option>
          <option value="ASAMBLAT">{PRODUCT_TYPE_ICONS['ASAMBLAT']} Asamblat</option>
        </select>
        {(search || typeFilter || alertOnly) && (
          <Button size="sm" onClick={() => { handleSearch(''); handleTypeFilter(''); handleAlertOnly(false) }}>✕ Resetează</Button>
        )}
      </div>
      ...
      <div className="bg-bg-surface border border-border rounded-xl">
        {isLoading
          ...
          : <Table columns={columns} data={filtered} keyFn={p => p.id} empty="Niciun produs găsit" page={page} perPage={20} onPageChange={setPage} />
        }
      </div>

      {canEdit && (
        <Modal open={open} onClose={closeModal} title={editing ? 'Editează Produs' : 'Produs Nou'}
          footer={<>
        ...
        >
          <div className="flex flex-col gap-4">
            <Input label="Denumire produs *" {...register('name', { required: true })} placeholder="ex: Lemn stejar, Dulap 3 uși..." error={errors.name ? 'Câmp obligatoriu' : ''} />
            ...
            <Input label="Descriere (opțional)" {...register('description')} placeholder="Scurtă descriere..." />
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-medium text-text-2">
                Stoc minim alertă <span className="text-text-3 font-normal ml-1">(0 = dezactivat)</span>
              </label>
              <input type="number" min="0" step="any" {...register('minStock')} placeholder="ex: 10"
                className="bg-bg-surface2 border border-border-2 rounded-md px-3 py-2 text-sm text-text outline-none focus:border-accent" />
              <p className="text-[11px] text-text-3">Vei primi alertă când stocul total scade sub această valoare</p>
            </div>
          </div>
        </Modal>
      )}
