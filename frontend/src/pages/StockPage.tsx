...
export default function StockPage() {
  ...
  const columns = [
    ...
    { key: 'type',      header: 'Tip',       render: (s: StockEntry) => productTypeBadge(s.product.type) },
    {
      ...
      render: (s: StockEntry) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-text-2">{s.warehouse.name}</span>
        </div>
      ),
    },
    {
    ...
    { key: 'unit', header: 'U.M.', render: (s: StockEntry) => <span className="text-text-2 text-xs">{unitLabel(s.product.unit)}</span> },
  ]
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-serif text-3xl font-light">Stoc Curent</h1>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        ...
        {(search || whFilter || typeFilter) && (
          <Button size="sm" onClick={() => { setSearch(''); setWhFilter(''); setTypeFilter('') }}>✕ Resetează</Button>
        )}
      </div>
      ...
      <div className="bg-bg-surface border border-border rounded-xl">
        {isLoading
          ...
          : <Table columns={columns} data={filtered} keyFn={s => s.id} empty="Niciun stoc găsit — adaugă o mișcare de recepție" />
        }
      </div>
    </div>
  )
